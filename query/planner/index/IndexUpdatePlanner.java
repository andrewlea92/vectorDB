...
import org.vanilladb.core.sql.VectorConstant;
import org.vanilladb.core.storage.index.IVF_FLATIndex; // Added for IVF_FLATIndex
import org.vanilladb.core.query.algebra.IVPlan;
import org.vanilladb.core.util.CoreProperties;
...
public class IndexUpdatePlanner implements UpdatePlanner {
	private static boolean benchingState = false;

	public static void setBenchingState(boolean state) {
		benchingState = state;
	}
	...
	public int executeInsert(InsertData data, Transaction tx) {
		...
		//System.out.println("Inserting into table: " + tblname);
		...
		for (IndexInfo ii : indexes) {
			if (benchingState) {
				...
				ivfIdx.insert((VectorConstant) fldValMap.get("i_emb"), rid);
				ivfIdx.close();
			} else {
